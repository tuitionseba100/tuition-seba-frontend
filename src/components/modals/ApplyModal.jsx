...
const ApplyModal = ({ show, onClose, tuitionCode, tuitionId }) => {
    ...
    useEffect(() => {
        if (show) {
            ...
            if (tuitionId) {
                fetchWithFallback(`https://tuition-seba-backend-1-vk5b.onrender.com/api/tuitionApply/get-auto-comment/${tuitionId}`)
                    .then(res => res.json())
    ...
    const handleVerification = async (values, setFieldValue) => {
        setIsVerifying(true);
        try {
            const checkRes = await fetchWithFallback('https://tuition-seba-backend-1-vk5b.onrender.com/api/regTeacher/check-apply-possible',
                {
                    method: 'POST',
            ...
            >
                ...
                <Formik
                    ...
                    onSubmit={async (values, { setSubmitting }) => {
                        ...
                        try {
                            const res = await fetchWithFallback('https://tuition-seba-backend-1-vk5b.onrender.com/api/tuitionApply/add-web',
                                {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
