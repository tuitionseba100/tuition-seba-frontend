...
const LeadPage = () => {
    ...
    const fetchDueTodayList = async () => {
        ...
        try {
            const response = await axios.get(
                'https://tuition-seba-backend-a0pb.onrender.com/api/lead/today-followups',
    ...
    const fetchUsers = async () => {
        try {
            const response = await axios.get(
                'https://tuition-seba-backend-a0pb.onrender.com/api/user/users',
                { headers: { Authorization: token } }
            );
    ...
    const fetchLeadRecords = async () => {
        setLoading(true);
        try {
            const response = await axios.get('https://tuition-seba-backend-a0pb.onrender.com/api/lead/getTableData', {
                params: {
                    page: currentPage,
    ...
    const fetchCardSummary = () => {
        axios.get('https://tuition-seba-backend-a0pb.onrender.com/api/lead/summary', {
            params: {
                tuitionCode: appliedFilters.tuitionCode,
                phone: appliedFilters.phone,
                status: appliedFilters.status
    ...
    const handleSaveRequest = async () => {
        ...
        try {
            if (editingId) {
                ...
                await axios.put(`https://tuition-seba-backend-a0pb.onrender.com/api/lead/edit/${editingId}`, updatedData, {
                    headers: { Authorization: token },
                });
                toast.success("Record updated successfully!");
            } else {
                await axios.post('https://tuition-seba-backend-a0pb.onrender.com/api/lead/add', updatedData, {
                    headers: { Authorization: token },
                });
    ...
    const handleDeleteRecord = async (id) => {
        if (window.confirm("Are you sure you want to delete this record?")) {
            try {
                await axios.delete(`https://tuition-seba-backend-a0pb.onrender.com/api/lead/delete/${id}`, {
                    headers: { Authorization: token },
                });
