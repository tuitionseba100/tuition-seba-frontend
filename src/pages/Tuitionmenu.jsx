...
const TuitionPage = () => {
    ...
    const fetchAlertData = async () => {
        try {
            ...
            const [alertRes, pendingRes, guardianRes] = await Promise.all([
                axios.get('https://tuition-seba-backend-1-lpfs.onrender.com/api/tuition/alert-today', { params: alertParams }),
                axios.get('https://tuition-seba-backend-1-lpfs.onrender.com/api/tuition/pending-payment-creation'),
                axios.get('https://tuition-seba-backend-1-lpfs.onrender.com/api/tuition/guardian-followup-today', { params: alertParams })
            ]);
    ...
    const handleAutoMigrate = async () => {
        if (selectedMigrationIds.length === 0) {
            toast.error("Please select at least one tuition to migrate.");
            return;
        }

        if (!window.confirm(`Are you sure you want to migrate ${selectedMigrationIds.length} selected tuition(s) from today to tomorrow?`)) {
            return;
        }
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post('https://tuition-seba-backend-1-lpfs.onrender.com/api/tuition/auto-migrate', {
    ...
    const handleSelectAllMigration = () => {
        if (selectedMigrationIds.length === tuitionNeedsUpdateList.length) {
            setSelectedMigrationIds([]);
        } else {
            setSelectedMigrationIds(tuitionNeedsUpdateList.map(t => t._id));
        }
    };
    ...
    const fetchTuitionRecords = async () => {
        setLoading(true);
        try {
            const response = await axios.get('https://tuition-seba-backend-1-lpfs.onrender.com/api/tuition/getTableData', {
                params: {
                    page: currentPage,
    ...
    const handleExportWithStatus = async () => {
        if (selectedExportStatus) {
            try {
                ...
                link.href = `https://tuition-seba-backend-1-lpfs.onrender.com/api/tuition/exportData?status=${selectedExportStatus}`;
                link.target = '_blank';
    ...
    const handleDeleteTuition = async (id) => {
        ...
        if (confirmDelete) {
            try {
                ...
                await axios.delete(`https://tuition-seba-backend-1-lpfs.onrender.com/api/tuition/delete/${id}`);
                        ...
                        >
                            ...
                            <td>
                                <span
                                    ...
                                    style={{ cursor: 'pointer' }}
                                    title="Click to view status history"
                                    className={`badge ${statusColors[tuition.status]?.bg || "bg-light"} ${statusColors[tuition.status]?.text || "text-dark"}`}
                                >
                                    {tuition.status}
                                </span>
                            </td>
