...
const TuitionPage = () => {
    ...
    const handleShowDetails = async (tuitionId) => {
        ...
        try {
            const response = await axios.get(`https://tuition-seba-backend-1-lpfs.onrender.com/api/tuition/${tuitionId}`);
            if (response.data) {
    ...
    const fetchTuitionApplyRecords = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get('https://tuition-seba-backend-1-lpfs.onrender.com/api/tuitionApply/getTableData', {
                params: {
                    page: currentPage,
    ...
    const fetchCardSummary = () => {
        axios.get('https://tuition-seba-backend-1-lpfs.onrender.com/api/tuitionApply/summary', {
            params: {
                tuitionCode: appliedFilters.tuitionCode,
                phone: appliedFilters.phone,
                status: appliedFilters.statusFilter
    ...
    const handleExportWithStatus = async () => {
        if (selectedExportStatus) {
            try {
                ...
                link.href = `https://tuition-seba-backend-1-lpfs.onrender.com/api/tuitionApply/exportData?status=${selectedExportStatus}`;
                link.target = '_blank';
    ...
    const handleSaveTuition = async () => {
        ...
        try {
            if (editingId) {
                await axios.put(`https://tuition-seba-backend-1-lpfs.onrender.com/api/tuitionApply/edit/${editingId}`, updatedTuitionData);
                toast.success("Tuition apply record updated successfully!");
            } else {
                await axios.post('https://tuition-seba-backend-1-lpfs.onrender.com/api/tuitionApply/add-web', updatedTuitionData);
    ...
    const handleDeleteTuition = async (id) => {
        ...
        if (confirmDelete) {
            setDeleting(true);
            try {
                await axios.delete(`https://tuition-seba-backend-1-lpfs.onrender.com/api/tuitionApply/delete/${id}`);
