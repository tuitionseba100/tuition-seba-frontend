...
const ExpensePage = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [expRes, summaryRes, todayRes, monthRes, overallRes] = await Promise.all([
                axios.get(`https://tuition-seba-backend-a0pb.onrender.com/api/expense/all?startDate=${start}&endDate=${end}&category=${selectedCategory}&page=${currentPage}&limit=50`, { headers }),
                axios.get(`https://tuition-seba-backend-a0pb.onrender.com/api/expense/summary?startDate=${start}&endDate=${end}&category=${selectedCategory}`, { headers }),
                axios.get(`https://tuition-seba-backend-a0pb.onrender.com/api/expense/summary?startDate=${todayStart}&endDate=${todayEnd}`, { headers }),
                axios.get(`https://tuition-seba-backend-a0pb.onrender.com/api/expense/summary?startDate=${monthStart}&endDate=${monthEnd}`, { headers }),
                axios.get(`https://tuition-seba-backend-a0pb.onrender.com/api/expense/summary`, { headers })
            ]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editMode) {
                await axios.put(`https://tuition-seba-backend-a0pb.onrender.com/api/expense/edit/${currentId}`, submitData, { headers });
                toast.success('Expense updated');
            } else {
                await axios.post('https://tuition-seba-backend-a0pb.onrender.com/api/expense/add', submitData, { headers });
    ...
    const handleDelete = async (id) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`https://tuition-seba-backend-a0pb.onrender.com/api/expense/delete/${id}`, {
