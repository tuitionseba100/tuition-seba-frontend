...
const UserPage = () => {
    ...
    const handleToggleLock = async (userId) => {
        ...
        try {
            await axios.put(`https://tuition-seba-backend-1-lpfs.onrender.com/api/user/toggle-lock/${userId}`, {}, {
    ...
    const fetchHistory = async (user) => {
        ...
        try {
            const response = await axios.get(`https://tuition-seba-backend-1-lpfs.onrender.com/api/user/history/${user._id}`, {
                headers: { Authorization: token }
            });
    ...
    const confirmDeleteUser = async () => {
        ...
        setError(null);
        try {
            await axios.delete(`https://tuition-seba-backend-1-lpfs.onrender.com/api/user/delete/${userToDelete}`, {
                headers: { Authorization: token }
            });
            await fetchUsers();
    ...
    const handleSaveUser = async () => {
        ...
        try {
            if (editingUser) {
                await axios.put(`https://tuition-seba-backend-1-lpfs.onrender.com/api/user/edit/${editingUser._id}`, newUser, {
                    headers: { Authorization: token }
                });
                toast.success('User updated successfully');
            } else {
                await axios.post('https://tuition-seba-backend-1-lpfs.onrender.com/api/user/register', newUser, {
                    headers: { Authorization: token }
                });
