...
const ApplyUpdates = () => {
    ...
    const fetchTuitionStatus = useCallback(async (phoneToSearch) => {
        ...
        try {
            const response = await fetchWithFallback(
                `https://tuition-seba-backend-1-vk5b.onrender.com/api/tuitionApply/getTuitionStatusesByPhone?phone=${phoneToSearch}`
            );
    ...
    const handleShowDetails = async (tuitionCode) => {
        ...
        try {
            const response = await fetchWithFallback(
                `https://tuition-seba-backend-1-vk5b.onrender.com/api/tuition/byCodePublic?tuitionCode=${tuitionCode}`
            );
            if (!response.ok) {
