...
import { Modal, Container, Row, Col, Card, Table, Form, Button, Spinner, Badge } from 'react-bootstrap';
...
export default function GlobalSearchModal({ show, onHide }) {
    ...
    const matchSummary = useMemo(() => {
        if (!results) return [];
        const summary = [
            { id: 'modal-guardianApplies', label: 'Guardian Applies', count: results.guardianApplies?.length || 0, icon: <FaUserPlus />, gradient: 'linear-gradient(135deg, #6a11cb 0%, #2575fc 100%)' },
            { id: 'modal-leads', label: 'Leads', count: results.leads?.length || 0, icon: <FaBullhorn />, gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
            { id: 'modal-phones', label: 'Spam/Best/Express', count: results.phones?.length || 0, icon: <FaShieldAlt />, gradient: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' },
            { id: 'modal-teachers', label: 'Teachers', count: results.teachers?.length || 0, icon: <FaUserTie />, gradient: 'linear-gradient(135deg, #00b09b 0%, #96c93d 100%)' },
            { id: 'modal-tuitions', label: 'Tuitions', count: results.tuitions?.length || 0, icon: <FaBriefcase />, gradient: 'linear-gradient(135deg, #f6d365 0%, #fda085 100%)' },
            { id: 'modal-tuitionApplies', label: 'Tuition Application', count: results.tuitionApplies?.length || 0, icon: <FaClipboardList />, gradient: 'linear-gradient(135deg, #84fab0 0%, #8fd3f4 100%)' },
            { id: 'modal-teacherPayments', label: 'Teacher Payments', count: results.teacherPayments?.length || 0, icon: <FaHandHoldingUsd />, gradient: 'linear-gradient(135deg, #a1c4fd 0%, #c2e9fb 100%)' },
            { id: 'modal-payments', label: 'Payments', count: results.payments?.length || 0, icon: <FaWallet />, gradient: 'linear-gradient(135deg, #cfd9df 0%, #e2ebf0 100%)' },
            { id: 'modal-refunds', label: 'Refunds', count: results.refunds?.length || 0, icon: <FaHistory />, gradient: 'linear-gradient(135deg, #ff9a9e 0%, #fecfef 100%)' },
            { id: 'modal-taskDatas', label: 'Task Data', count: results.taskDatas?.length || 0, icon: <FaLayerGroup />, gradient: 'linear-gradient(135deg, #a18cd1 0%, #fbc2eb 100%)' },
        ];
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const endpoint = searchType === 'phone'
                ? `https://tuition-seba-backend-16yx.onrender.com/api/generalSearch/phone/${searchTerm}`
                : `https://tuition-seba-backend-16yx.onrender.com/api/generalSearch/tuition/${searchTerm}`;
            ...
            <Card id={id} className="mb-4 shadow-sm mt-4" style={{ borderRadius: '15px', overflow: 'hidden' }}>
                ...
                <Card.Body className="p-0">
                    <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                        <Table striped bordered hover responsive className="mb-0" style={{ fontSize: '1.1rem' }}>
        ...
        <Modal show={show} onHide={onHide} size="xl" scrollable centered>
            <Modal.Header closeButton className="bg-primary text-white">
                <Modal.Title className="fw-bold d-flex align-items-center">
                    <FaSearch className="me-2" /> Global Search Portal
                </Modal.Title>
            </Modal.Header>
            <Modal.Body className="bg-light">
                <Container fluid className="px-2 py-2">
                    {/* Refined Search Card */}
                    <Card className="mb-4 shadow-sm border-0 bg-white" style={{ borderRadius: '15px', overflow: 'hidden' }}>
                    ...
                    {results && !loading && matchSummary.length > 0 && (
                        <div className="mt-4 mb-4">
                            <div className="d-flex align-items-center mb-3">
                                <h4 className="fw-bold mb-0 text-dark" style={{ fontSize: '1.5rem' }}>Search Insights</h4>
                                <hr className="flex-grow-1 ms-3 opacity-10" />
                            </div>
                            <Row className="g-2">
                                {matchSummary.map((item) => (
                                    <Col key={item.id} xs={6} sm={4} md={3} lg={2.4}>
                                        <div
                                            onClick={() => scrollToCategory(item.id)}
                                            className="summary-card-elegant-modal h-100 p-2 rounded-3 shadow-sm d-flex flex-column align-items-center justify-content-center text-center transition-all"
                                            style={{
                                                background: 'white',
                                                cursor: 'pointer',
                                                minHeight: '90px',
                                        ...
                                        >
                                            <div className="label-text mb-1 text-uppercase fw-black text-dark" style={{ fontSize: '0.8rem', letterSpacing: '0.02em' }}>
                                                {item.label}
                                            </div>
                                            <div className="count-text h2 mb-0 fw-black" style={{ color: '#1a252f' }}>
                                                {item.count}
                                            </div>
                                        </div>
                                    </Col>
                    ...
                    {loading && (
                        <div className="text-center my-5 py-5">
                            <Spinner animation="border" variant="primary" style={{ width: '3rem', height: '3rem', borderWidth: '0.25em' }} />
                            <p className="mt-3 fw-bold text-primary h6">Searching records...</p>
                        </div>
                    )}

                    {results && !loading && (
                        <div className="results-container">
                            {renderTable("modal-guardianApplies", "Guardian Applications", results.guardianApplies, [
                            ...
                            {renderTable("modal-leads", "Leads", results.leads, [
                                { key: 'name', label: 'Name' },
                                { key: 'phone', label: 'Phone' },
                                { key: 'status', label: 'Status' },
                            ...
                            {renderTable("modal-phones", "Spam/Best/Express", results.phones, [
                                { key: 'phone', label: 'Phone' },
                                {
                                    label: 'Status',
                                    render: (item) => (
                            ...
                            {renderTable("modal-teachers", "Registered Teachers", results.teachers, [
                            ...
                            {renderTable("modal-tuitions", "Tuition Records", results.tuitions, [
                                { key: 'tuitionCode', label: 'Code' },
                            ...
                            {renderTable("modal-tuitionApplies", "Tutor Applications", results.tuitionApplies, [
                            ...
                            {renderTable("modal-teacherPayments", "Teacher Payments", results.teacherPayments, [
                                { key: 'tuitionCode', label: 'Code' },
                            ...
                            {renderTable("modal-payments", "Payments", results.payments, [
                                { key: 'tuitionCode', label: 'Code' },
                            ...
                            {renderTable("modal-refunds", "Refund Requests", results.refunds, [
                                { key: 'tuitionCode', label: 'Code' },
                                { key: 'personalPhone', label: 'Teacher No' },
                                { key: 'amount', label: 'Amount' },
                                { key: 'status', label: 'Status' }
                            ...
                            {renderTable("modal-taskDatas", "Task Data", results.taskDatas, [
                                { key: 'tuitionCode', label: 'Code' },
                            ...
                            {matchSummary.length === 0 && (
                                <div className="text-center my-5 py-5 bg-white border shadow-sm rounded-4">
                                    <FaSearch className="text-muted opacity-25 display-3 mb-3" />
                                    <h4 className="fw-bold text-dark">No matches found</h4>
            ...
            <style>
                {`
                    ...
                    .summary-card-elegant-modal:hover {
