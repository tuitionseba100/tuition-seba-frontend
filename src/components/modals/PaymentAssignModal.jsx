...
const PaymentAssignModal = ({ show, onHide, payment, fetchPaymentRecords, fetchAlertData }) => {
    ...
    useEffect(() => {
        if (show) {
            setAssignedTo(payment?.assignedTo || '');
            fetchUsers();
        }
    }, [show, payment]);
    ...
    const handleAssign = async () => {
        ...
        const confirmAssign = window.confirm(`Are you sure you want to assign this payment to ${assignedTo}?`);
        if (!confirmAssign) return;

        setLoading(true);
        try {
            const username = localStorage.getItem('username');
            await axios.put(`https://tuition-seba-backend-1-lpfs.onrender.com/api/payment/edit/${payment._id}`, {
                assignedTo: assignedTo,
                updatedBy: username
            });
            toast.success(`Payment assigned to ${assignedTo} successfully!`);
            if (fetchPaymentRecords) fetchPaymentRecords();
    ...
    return (
        <Modal show={show} onHide={onHide} centered>
            <Modal.Header closeButton className="bg-primary text-white">
                <Modal.Title>Assign Employee - {payment?.tuitionCode}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
...
export default PaymentAssignModal;
